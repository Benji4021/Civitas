...
func _can_drop_data(_pos: Vector2, data) -> bool:
	var ok: bool = typeof(data) == TYPE_DICTIONARY and data.get("type") == "building" and (data.get("scene") is PackedScene)
	if not ok:
		return false
...
func _origin_under_mouse(data: Dictionary) -> Vector2i:
	...
	return Vector2i(mouse_tile.x - int((fp.x - 1) / 2), mouse_tile.y - int((fp.y - 1) / 2))
...
func _set_ground_occupied(tile: Vector2i, on: bool) -> void:
	if on:
		if not original_ground.has(tile):
			original_ground[tile] = {"source": ground.get_cell_source_id(tile), "atlas": ground.get_cell_atlas_coords(tile), "alt": ground.get_cell_alternative_tile(tile)}
